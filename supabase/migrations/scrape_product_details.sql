...
function mapMonth(nameRaw) {
  const n = nameRaw.toLowerCase().replace(/\.$/, '').normalize('NFD').replace(/[\u0300-\u036f]/g, ''); // strip diacritics (e.g., févr → fevr, märz → marz)
...
function parseTopCategoryRanks(html) {
  const text = html.replace(/<[^>]+>/g, ' ').replace(/\s+/g, ' ');
...
function decideBestsellerByRanks(ranks) {
...
function detectAvailability(html) {
  ...
  if (/(in stock|disponibile|artikel auf lager|en stock|en existencia)/i.test(plain)) {
    return {
      code: 'IN_STOCK',
      text: plain.match(/(in stock|disponibile|artikel auf lager|en stock|en existencia)/i)?.[0] || null
    };
  }
  if (/(available to ship|usually ships|ships within|spedizione|disponibile tra|verfügbar in|expédition sous|disponible en)/i.test(plain)) {
    return {
      code: 'AVAILABLE_SOON',
      text: plain.match(/(available to ship[^.]*|usually ships[^.]*|ships within[^.]*|spedizione[^.]*|disponibile tra[^.]*|verfügbar[^.]*|expédition sous[^.]*|disponible en[^.]*)/i)?.[0] || null
    };
  }
  if (/(temporarily out of stock|temporalmente esaurito|derzeit nicht auf lager|temporairement en rupture|temporalmente sin stock)/i.test(plain)) {
    return {
      code: 'OUT_OF_STOCK',
      text: plain.match(/(temporarily out of stock|temporalmente esaurito|derzeit nicht auf lager|temporairement en rupture|temporalmente sin stock)/i)?.[0] || null
    };
  }
  if (/(currently unavailable|attualmente non disponibile|derzeit nicht verfügbar|actuellement indisponible|actualmente no disponible)/i.test(plain)) {
    return {
      code: 'UNAVAILABLE',
      text: plain.match(/(currently unavailable|attualmente non disponibile|derzeit nicht verfügbar|actuellement indisponible|actualmente no disponible)/i)?.[0] || null
    };
  }
  return {
    code: null,
...
const bindingMap = [
  {
    rx: /paperback|copertina\s*flessibile|taschenbuch|broché|tapa\s*blanda/i,
    val: "paperback"
  },
  {
    rx: /hardcover|copertina\s*rigida|gebundenes\s*buch|relié|tapa\s*dura/i,
    val: "hardcover"
  }
];
const languageKeys = [
  {
    rx: /language|lingua|sprache|langue|idioma/i,
    key: "language"
  }
];
...
function normalizeTrimFromText(text) {
...
function detectInterior(html, title, price, pages) {
  const t = `${title}\n${html}`.toLowerCase();
...
async function fetchWithKey(client, userId, asin, country, url) {
  const nowIso = new Date().toISOString();
  const { data: keys } = await client.from("scraper_api_keys").select("id, api_key, status, credits, max_credits, cost_per_call, last_used_at, success_count, failure_count, last_success_at, cooldown_until").eq("user_id", userId).eq("service_name", "scraperapi");
...
serve(async (req)=>{
  ...
  try {
    ...
    const country = (payload?.country || "com").toLowerCase();
    const userId = payload?.userId;
    if (!asin || !userId) {
      return new Response(JSON.stringify({
        success: false,
        error: "Missing asin or userId"
      }), {
        status: 400,
        headers: {
          ...corsHeaders,
    ...
    const { data: asinRow } = await client.from("asin_data").select("id, price, title, is_bestseller, is_bestseller_miss_count").eq("asin", asin).eq("user_id", userId).maybeSingle();
    const price = Number(asinRow?.price) || 0;
    const title = String(asinRow?.title || "");
    // Extract fields
    let page_count = extractPageCount(html);
    if (Number.isFinite(page_count) && page_count < 1) page_count = null;
    // Try to find the line that contains dimensions
    let dimensions_raw = null;
    ...
    if (asinRow?.id) {
      try {
        ...
        await client.from("asin_data").update({
          page_count,
          dimensions_raw,
          trim_size,
          binding,
    ...
    return new Response(JSON.stringify({
      ...
      data: {
        ...
        interior_type,
        interior_confidence,
        is_bestseller: hasRankData ? top1 : null,
        stock_status: avail.text || null,
        availability_code: avail.code || null
      }
    }), {
      headers: {
        ...corsHeaders,
